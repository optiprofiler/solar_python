...
def solar_collect_info():
    ...
    The returned rows are read from ``runtime/solar/metadata/probinfo.csv``.
    This file is generated by ``scripts/collect_info.py`` by loading each
    enabled SOLAR problem through ``solar_load`` and recording the resulting
    OptiProfiler ``Problem`` fields.
